...
export function encodeRSA (_text: string): string {
    const _buffer = Buffer.from(_text);
    return Crypto.publicEncrypt(_publicKey, _buffer).toString();
}
export function decodeRSA (_text: string): string {
    const _buffer = Buffer.from(_text, 'hex');
    return Crypto.privateDecrypt(_privateKey, _buffer).toString();
}
export default {
